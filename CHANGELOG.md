...
- **入口铁律（治"跳步骤"）**：`SKILL.md` Step 2 从"列阶段名"改为 **"逐阶段加载 + 卡点等待 + 不可跳步 + 进度可见"** 的强制循环；8 个 phase 顶部"可插拔模块"→"必经一步（不可跳过/合并）"。经对抗验证补漏：进度行 `【N/8】` 下沉到每个 phase 文件顶部；明确 **Phase 1 ≠ Step 1**（受众卡/完成卡不被"已加载文件"吞掉）；phase-05 补顶部 `强制等待锚点`，与其余 7 阶段结构对称。

### 移除 / 合并（版式库去重，治"版式选不对"）
- 删重复版式：**I-02**（深色宣言→并入 T-01）、**I-03**（反差双栏→并入 S-08）、**V-01**（KPI 看板→并入 S-09）。
- 删 **Ch 伪系**：数据图表归 V 系 + `charts.py`，不再当独立版式系。
- **SPC 系**不再谎称"骨架在 common.md 末尾特殊页模板"（实不存在）——封面/目录/章节由 `themes.py` 生成。
- 修孤儿 **T-02**：补"白底大字宣言/金句"选择路径（原先零路径指向）。
- **HERO 家族统一命名**（S-07 阶梯递进→HERO-3）；选择速查 HERO 三行收为一行。
- KPI 数值统一墨色（S-09 去红绿）；I-05 窗口期大数字去红（紧迫性靠红"当前"节点+警示标题双通道）。
- 版式库：声明 37→**34 ID**，骨架实存 **31 个**（i3 s12 e4 v3 t3 a3 g3）。

### 新增（治"版式选不对"·选版式权威化 + 防平淡硬门禁）
- **选版式单一权威表**：`layout-library §版式选择速查` 设为唯一决策表 + 标注「首选核心集」（治选项过载）；`phase-05` 删内联映射、改为引用；`designer-rules` 路径A 的封面/目录/章节改走 `themes.py`（删除与之打架的旧 cover→I-01 映射）。
