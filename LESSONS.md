# LESSONS · 经验积累文档（历史经验归档）

> **注意：本文件不在运行时加载链路中，不会消耗token。**
> 本文件是供人类阅读的经验积累，包含方法论场景索引和工具链技术细节。
> AI执行时不需要读取本文件，所有执行规则已分散到对应的 Phase 文件和 references 中。

> **⚠️ 历史归档说明**：本文件是**方法论经验的历史归档**。下文各章节标注的
> `**文件**:references/xxx.md` 是**历史记录的指针**——技能演进后，这些场景的可执行规则
> 已重构、合并并分散到现行的 `references/coach-engine.md`、`references/phase-01.md`～`phase-08.md`
> 等文件中，**不再以独立的 references 文件单独承载**。需要落地规则时，请按下方「使用索引」的
> 阶段映射去对应 Phase / coach-engine 文件查阅；本文件中的文件名仅供理解经验来源脉络。

---


> 本文件是技能的**方法论索引**。24 个场景来源于多次项目实战,每个配 Trigger Point 和跳转链接。
> **使用方式**:在对应阶段或遇到对应信号时,按「使用索引」跳转到对应 Phase / coach-engine 文件读完整内容。

---

## 📍 必问探测类(场景 1-12)

**历史来源**:`references/probing-scenarios.md`（已重构，现行规则见 `references/coach-engine.md` 与各 Phase 文件）

### 强制 4 场景(Phase 0.5 必问)

| # | 场景 | Trigger Point |
|---|---|---|
| 1 | 规模语境校准 | 进入 Phase 1 调研前 |
| 2 | 受众心理动线建模 | Phase 2 Ghost Deck 前 |
| 3 | ★ 第三章灵魂页开放性取向探测 | 设计方案 Part 核心框架前(最重要) |
| 7 | 成长带诚实性校验 | 方案涉及未来规划/远景时 |

### 可选 8 场景(按需触发)

| # | 场景 | Trigger Point |
|---|---|---|
| 4 | 诊断页问题定性校准 | 设计"现状诊断"类内容页前 |
| 5 | 多概念并列结构辨识 | 出现"XX+YY"并列时 |
| 6 | 数字/承诺诚实度校验 | 方案里出现任何具体数字时 |
| 8 | 节奏类型确认 | Phase 4 策划版式前 |
| 9 | 战略轴心页上下咬合 | 设计轴心页时 |
| 10 | 跨 Part 呼应锚点埋设 | Phase 4 策划跨 Part 呼应时 |
| 11 | bullets 风格场景匹配 | Phase 3 填充 bullets 时 |
| 12 | ★ 矛盾一致性全扫描 | Phase 6 QC 交付前最后一关 |

---

## 🎯 专业判断类(场景 13-14)

**历史来源**:`references/professional-judgment.md`（已重构，现行规则分散在各 Phase 文件）

| # | 场景 | Trigger Point |
|---|---|---|
| 13 | ★ "懂客户"的专业边界(极易滑向讨好) | 涉及"延续 vs 重构"决策时 |
| 14 | ★ 甲乙方责任边界判断(不揽活也是专业) | 涉及"谁来做"的分工时 |

**核心原则**:专业判断不搬家。知道哪些事不该做,和知道哪些事该做,同等重要。

---

## 🎨 规范执行类(场景 15 + 架构修正)

**历史来源**:`references/brand-discipline.md`（已重构，现行规则见 `references/brand-spec/brand.md` 与 `references/designer-rules.md`）

| # | 场景 | Trigger Point |
|---|---|---|
| 15 | ★★ 颜色使用必须从品牌规范读取,不得自定义 | 设计任何涉及颜色的页面 |
| — | 架构级修正:brand.md 作为唯一真源 | 全技能生命周期 |

**四项强制纪律**:
1. 前置读取 brand.md §2/§3/§7
2. designer 阶段硬性查表
3. QC 阶段强制合规审计
4. Skill 文档内容严禁具体色值

---

## 🔧 工程教训类(场景 19-23)

**历史来源**:`references/engineering-lessons.md`（已重构，现行规则见 `references/pptx-native-rules.md`、`references/quality-audit-protocol.md` 与 `references/phase-07.md`）

| # | 场景 | Trigger Point |
|---|---|---|
| 19 | ★ 大规模 QC 工具预算规划 | 50+ 页 SVG/PPT 的视觉 QC |
| 20 | ★★★ SVG 为真源原则 | 任何需要输出多种格式的交付任务 |
| 21 | ★ EMF 嵌入不可用于"可编辑"承诺 | 用户要求"可编辑 PPT""原生形状" |
| 22 | ★★ 通用 SVG→PPT 转换器的坑 | 想自己写简单 SVG→PPT 转换器时 |
| 23 | ★★ 深度调研再动手 | 遇到反复踩坑的技术问题 |

**V4.0 废弃的场景**:
- 场景 17 问法保留(输出格式三选一),实现升级为调用工具链
- 场景 18(分批合并 PPTX)完全废弃,移入 LEGACY 章节

---

## 📖 使用索引

### 按执行阶段查找

| 阶段 | 必读场景 |
|---|---|
| Phase 0 | 17(三选一问法)、20(SVG 真源) |
| Phase 0.5 | 1, 2, 3, 7(强制 4 场景) |
| Phase 0.5b | 15(规范真源) |
| Phase 1 调研 | 13(继承/重构判断) |
| Phase 2 叙事 | 10(跨 Part 呼应) |
| Phase 3 大纲 | 4(诊断定性)、11(bullets 风格)、14(甲乙方分工) |
| Phase 4 策划 | 5(多概念辨识)、8(节奏类型)、9(轴心咬合) |
| Phase 5 SVG | 15(颜色真源) |
| Phase 5.5 PPT | 20, 21, 22(SVG 真源/可编辑纪律/不要自己写转换器) |
| Phase 6 QC | 6(数字诚实)、12(矛盾扫描)、19(QC 预算) |

### 按问题信号查找

| 遇到什么情况 | 翻到哪个场景 |
|---|---|
| 方案里好几处具体数字 | 场景 6 |
| 方案出现"XX+YY"并列 | 场景 5 |
| 客户现有系统要"延续还是重构" | 场景 13 |
| 甲乙方分工拿不准 | 场景 14 |
| 想自己定义新色值 | 场景 15(不要!查 brand.md) |
| 想自己写 SVG→PPT 转换器 | 场景 22(不要!用工具链) |
| 已经踩坑 2 次以上 | 场景 23(停下来调研) |
| 承诺"可编辑 PPT" | 场景 21(必须用 svg_to_pptx 路径) |
| 有 SVG 又要做 PPT | 场景 20(SVG 是唯一真源) |
| 50+ 页要做 QC | 场景 19(四阶段工具预算) |

---

## 跨项目复用原则

**"答案"项目相关,"提问方式"跨项目复用。**

同一个场景在不同项目(汽配 / 教育 / 金融 / 医疗 / 消费品)里,问的是同一组问题,但答案千差万别。

本技能的价值在于:
1. 保证关键节点不会漏问(探测类)
2. 保证硬判断不被软化(专业类)
3. 保证规范不被稀释(规范类)
4. 保证不重复踩坑(工程类)

---

## 场景增补流程

如果在未来项目中发现新的关键场景:

1. 项目结束时单独写一份 `SKILL_UPGRADE_NOTES_YYYYMMDD.md`
2. 提炼 Trigger Point / Open Questions / Usage / Cost of Skipping
3. 在下一次技能升级时整合进对应的 `references/*.md`
4. 更新本 `LESSONS.md` 索引

**不要边做项目边改技能**:项目当下的修改容易带入项目特殊性,沉淀要等项目结束后的冷思考。


---

## 工具链技术细节（供调试参考）

ppt-master 工具链的核心创新(`estimate_text_width` 等实现在 `assets/svg_to_pptx/drawingml_utils.py`):

> 下方代码块为历史摘录,系数可能与源码漂移;**一切以源码 `drawingml_utils.py` 为准**。

```python
# baseline 偏移系数(经验值)
box_y = svg_y - font_size * 0.85

# 字符级精确宽度估算
def estimate_text_width(text, size, weight):
    width = 0
    for ch in text:
        if is_cjk_char(ch):
            width += size          # 中文 = 1em
        elif ch == ' ':
            width += size * 0.3
        elif ch in 'mMwWOQ':
            width += size * 0.75
        elif ch in 'iIlj1!|':
            width += size * 0.3
        else:
            width += size * 0.55
    if weight in ('bold', '600', '700', '800', '900'):
        width *= 1.05
    return width

# 关键 XML 配置
# wrap="none" 禁止换行 + spAutoFit 自动贴合 + lIns/tIns/rIns/bIns="0" 去内边距
```

详细实现见 `assets/svg_to_pptx/` 源码。**无需修改**,直接使用。


## 实战复盘 · 大体量方案册（2026 制造业 MES 案）

- **tofu 字符反复踩**：✕/✓/①②③/↕ 在等宽或无字形字体渲染成 □。→ 已加 `tools/lint_svg.py` 预检；规范见 SKILL.md 附录。
- **中文双引号 “” 致 SyntaxError**：批量脚本里最隐蔽的坑。→ 一律「」。
- **图标语义错配**：gearmold 画成"太阳"还一图三用。→ 已修正 gearmold（模具）、新增 injection（注塑机）；新增 `tools/icons_preview.py` 强制肉眼校验。
- **页码/总数分散**：50/64/57 并存。→ 统一占位 + `tools/pageno_fill.py` 集中回填。
- **内容对资料的出入**：架构系统成员、产线命名、边界口径，靠逐页对《项目基线》才发现。→ 固化"基线 + 双轨审计"流程。
- **有效做法**：项目基线（单一事实源）、灵魂句统领、占位机制、三类适配统一口径、以客户评审图为锚——建议沿用。


---

## 场景 2026-07 · FIFA26 咨询版式实战（D 层起源）

**文件**: references/svg-skeleton-d.md §4 / assets/components/consulting.py（现行规则所在）

1. **竖排文字**：`<text transform="translate(…) rotate(-90)">` 走 svg_to_pptx 后整体漂移到页顶
   （白字落白底，灰度差质检不可见——质检盲区）。现行规则：D 层不做竖排；chrome（页眉/页脚/左缘）
   归 skeleton-common 与 themes.py，D 层禁止改动（v2.5.1 用户裁定，废弃逐字侧标方案）。
2. **细段标注**：f-string 拼接漏空格产出 "1/44"（"1/4" 段 + 数值 4）。现行规则：标注文字自带
   空格 + `tick_label(level)` 错层。
3. **符号字形**：`✓` 命中 lint_svg tofu 高危名单。现行规则：`check()` 两段直线画对勾。

4. **密度**：v2.5 初版每页只放单个 X-90 元素，被用户判「太过简约」。现行规则：X-90 为元素级版式，
   每页 = 主视觉 + ≥1 辅助层 + 标注层 + So-What（svg-skeleton-d §2 组合配方 / §3 密度纪律）。
