...
def _build_text_runs(
    ...
    Note: dy-based line breaks are handled separately by _has_dy_tspan_linebreaks
    and convert_text, which splits the <text> into multiple ShapeResult objects.
    """
...
def _has_dy_tspan_linebreaks(elem: ET.Element) -> bool:
    """Return True if this <text> element uses <tspan dy=...> for multi-line layout.

    When tspan elements carry a dy attribute (e.g. dy="1.4em"), the intent is
    multi-line text. DrawingML cannot replicate dy offsets inside a single
    TextBox, so we split them into separate TextBox shapes instead.
    """
    for child in elem:
        child_tag = child.tag.replace(f'{{{SVG_NS}}}', '')
        if child_tag == 'tspan':
...
def _parse_em_or_px(value: str, font_size: float) -> float:
    """Parse a CSS length value (em or px) into SVG pixel units."""
    value = value.strip()
...
def _build_tspan_line_shapes(
    ...
    """Split <tspan dy=...> children into independent TextBox ShapeResults.

    Each tspan with a dy offset becomes its own TextBox at the correct
    absolute Y position, giving reliable multi-line layout in PPTX.
    """
    ...
    for i, child in enumerate(children):
        ...
        # Inherit / override per-tspan attrs
        run_attrs = dict(parent_attrs)
        if child.get('font-weight'):
            run_attrs['font_weight'] = child.get('font-weight')
        if child.get('fill'):
        ...
        # Override x if tspan specifies one
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    Two paths:
    - If <tspan dy=...> children are detected: split into multiple independent
      TextBox shapes (one per tspan line), returned as a group.
    - Otherwise: single TextBox with all runs in one paragraph (original path).
    """
    x = ctx_x(_f(elem.get('x')), ctx)
    y = ctx_y(_f(elem.get('y')), ctx)
    ...
    # ---------------------------------------------------------------
    # Path A: tspan dy multi-line → split into independent TextBoxes
    # ---------------------------------------------------------------
    if _has_dy_tspan_linebreaks(elem):
        line_shapes = _build_tspan_line_shapes(
            elem, x, y, parent_attrs, fonts, algn, text_anchor, effect_xml, ctx,
        )
        if not line_shapes:
            return None
        if len(line_shapes) == 1:
            return line_shapes[0]
        # Wrap multiple lines in a flat XML string (no grpSp needed —
        # the caller in convert_svg_to_slide_shapes just appends xml strings)
    ...
    # ---------------------------------------------------------------
    # Path B: single TextBox (original behaviour)
    # ---------------------------------------------------------------
