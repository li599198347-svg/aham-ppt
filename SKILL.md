...
name: aham-ppt
description: >
  全流程AI制作PPT演示文稿技能，克制专业、风格可选 A/B/C 三档（Aham UI v6.1 · Office 轨 · PPT）。
...
> **版本 v2.2**（表现力增强 · 防平淡），在 v2.1 基础上新增 **9 类数据图表组件 `charts.py`、C 高表现力档（深色模板 `cover_dark/section_dark/quote_dark`）、防平淡规则链**（数据强制图表化 + 图形化占比下限 + 防平淡审计；详见 `CHANGELOG.md`）。
>
...
> 4. **进度可见**：每个阶段开头先输出一行 `【阶段 N/8 · 名称 · 已加载 phase-0N.md】`，让执行轨迹可被监督。

阶段一览（**名称仅供索引，不能据此直接产出**；详细指引各见对应文件）：

| # | 阶段 | 文件 |
|---|---|---|
| 1 | 规范加载（即本 Step 1） | `references/phase-01.md` |
